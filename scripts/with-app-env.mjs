...
function resolveCommand(command, env) {
  ...
  env[pathKey] = current.split(delimiter).includes(binDir)
    ? current
    : `${binDir}${current ? delimiter + current : ""}`;
  const local = join(binDir, command);
...
function main(argv) {
  ...
  if (command === "vite" && resolved === "vite") {
    console.error(
      "[with-app-env] vite is not installed. In this folder run:\n  npm install\n  npm run dev",
    );
    process.exit(127);
  }
