...
**MuJoCo, for real.** The notebook's `mujoco` cell loads the same menagerie G1 in MuJoCo 3.13 on the molab box: 36 dof, 29 position actuators, 3 s of physics under gravity at the `stand` keyframe in ~50 ms, pelvis holds at 0.79 m — a `weave.op`. The browser café replays the posed body; **it does not run a MuJoCo environment.** **The G1 walks in MuJoCo with a pretrained RL policy** (notebook cell `rlwalk`): MuJoCo Playground's `g1_policy.onnx` joystick locomotion policy, run headless in C MuJoCo on the molab box — 1.43 m in 2 s at `vx=0.8`, pelvis height held at 0.72 m. `g1_walker.step(vx, vy, wz)` returns body poses; streaming those into the café's per-body glTF nodes is the next step (branch `mujoco-walk`). In the browser the legs currently swing procedurally about MuJoCo-derived hip/knee pivots.
